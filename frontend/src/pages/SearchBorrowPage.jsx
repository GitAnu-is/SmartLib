...
export function SearchBorrowPage({ onNavigate }) {
  ...
  useEffect(() => {
    const selectedBookId = localStorage.getItem('searchBorrowBookId')
    if (!selectedBookId || books.length === 0) return

    const fromList = books.find((b) => String(b._id) === String(selectedBookId))
    if (fromList) {
      setSelectedBook(fromList)
      localStorage.removeItem('searchBorrowBookId')
      return
    }

    const loadBook = async () => {
      try {
        const data = await fetchBookById(selectedBookId)
        if (data?._id) {
          setSelectedBook(data)
        }
      } catch {
        // Ignore and keep normal search flow when book lookup fails.
      } finally {
        localStorage.removeItem('searchBorrowBookId')
      }
    }
