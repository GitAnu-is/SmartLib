    ...
    return requests
      ...
      .map((r) => {
        ...
        return {
          ...
          totalFine,
          status: paidFines.includes(r.id) ? 'paid' : 'unpaid',
        }
      })
      .filter(Boolean)
  }, [requests, paidFines])
  ...
  return (
    <div className="min-h-screen bg-light p-4 sm:p-8">
              ...
              {!booksLoading &&
                filteredBooks.map((book) => {
                  ...
                  return (
                    ...
                    >
                      <BookCoverImage title={book.title} coverColor={book.coverColor || 'bg-teal'} coverImageUrl={book.coverImageUrl} width="w-full" height="h-40" rounded="rounded-2xl" iconSize={48} />
              ...
              <div className="divide-y divide-gray-100">
                ...
                {filteredRequests.map((request) => (
                  <div key={request.id} className="p-6 flex items-center gap-4">
                    <BookCoverImage title={request.bookTitle} coverColor={request.coverColor} coverImageUrl={request.coverImageUrl} width="w-16" height="h-20" rounded="rounded-xl" iconSize={24} className="flex-shrink-0" />
                ...
                <div className="space-y-4">
                  {waitingList.map((item) => (
                    <div key={item.id} className="flex items-center gap-4 p-4 bg-light rounded-2xl">
                      <BookCoverImage title={item.bookTitle} coverColor={item.coverColor} coverImageUrl={item.coverImageUrl} width="w-16" height="h-20" rounded="rounded-xl" iconSize={24} className="flex-shrink-0" />
                      <div className="flex-1">
                        ...
                        <tr key={fine.id} className="hover:bg-light/50">
                          ...
                          <td className="p-4 text-right">
                            {fine.status === 'paid' ? (
                              ...
                              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setSelectedFine(fine); setShowPayFineModal(true); }} className="px-4 py-2 bg-coral text-white rounded-full text-sm font-bold shadow-lg shadow-coral/30">
                                Pay Fine
                              </motion.button>
                            )}
                          </td>
              ...
              {fines.some(f => f.status === 'unpaid') && (
                <div className="p-6 bg-light border-t border-gray-100 flex justify-between items-center">
                  ...
                  <span className="text-2xl font-extrabold text-coral">
                    {formatLkr(fines.reduce((sum, f) => f.status === 'unpaid' ? sum + (Number(f.totalFine) || 0) : sum, 0))}
                  </span>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <BookCoverImage title={selectedBook.title} coverColor={selectedBook.coverColor || 'bg-teal'} coverImageUrl={selectedBook.coverImageUrl} width="w-24" height="h-32" rounded="rounded-2xl" iconSize={40} />
                  <button
                    onClick={() => setSelectedBook(null)}
      ...
      <PayFineModal
        ...
        fine={selectedFine}
        onSuccess={(fineId) => setPaidFines(prev => [...prev, fineId])}
      />
    </div>
  )
}
